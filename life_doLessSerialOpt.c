...
void test_if_UpdateCellsOnBlockisok(int N, int** state, int** state_new, int** temp){
  testUpdateCellsOnBlock(1, N+1, 1, N+1, state, state_new, temp);
  return;
}
void next_gen(int N, int** state, int** state_new, int** temp){
  for(iStart=1; iStart+blockSz<N+1; iStart+=blockSz){
    ...
    //Update remaining j
    UpdateCellsOnBlock(iStart, iEnd, jStart, N+1, state, state_new, temp);
  }
  //Update remaining i
...
void next_gen_unroll(int N, int**  state, int**  state_new){
  //get neighbours
  //get first row
  for(int i=1; i < N+1; i++){
    int j=1;
